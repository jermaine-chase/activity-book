...
export class WordscrambleComponent implements OnInit {
  ...
  doWork = function() {
    ...
    let imageUrlExists = $('#imageUrl').val() !== '';
    if (wList.length > 0) {
      ...
      for (let idx = 0; idx < wList.length; idx++) {
        out += '<div class="row">';
        out += '<div class="col-6 outBody text-left">'+this.scrambleWord(wList[idx])+'</div><div class="col-6 outBody text-left">'+this.makeDashes(wList[idx])+'</div>';
        out += '</div>';
      }
      if (imageUrlExists) {
  ...
  getWordList = function(sort) {
    let wordList = $('#wordListIn').val().toLowerCase();
    let wordArray = [];
